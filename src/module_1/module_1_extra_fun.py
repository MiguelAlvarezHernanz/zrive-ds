...
def plot_comparative_temperature(results):
    ...
    base_colors = {"Madrid": "red", "London": "blue", "Rio": "green"}
    ...
    for city, data in results.items():
        ...
        plt.plot(
            annual_max_temp.index,
            annual_max_temp,
            label=f"{city} (max)",
            color=max_color,
        )
        plt.plot(
            annual_min_temp.index,
            annual_min_temp,
            label=f"{city} (min)",
            color=min_color,
        )
...
def plot_comparative_precipitation(results):
    """
    Plots the accumulated precipitation for each semester (April-September and 
    October-March) for each city.
    
    Parameters:
        results (dict): Dictionary containing processed data for each city.
    """
    semesters = {"April-September": (4, 10), "October-March": (10, 4)}
